...
static inline int find_contact_from_impu(impurecord_t* impu, str* search_aor, ucontact_t** scontact) {
    ...
    while (impucontact) {
        ...
        if (impucontact->contact) {
            ...
            c_term += 1;
            c_searchlen = impucontact->contact->c.len - (c_term-impucontact->contact->c.s);

            LM_DBG("Comparing [%.*s] and [%.*s]\n", i_searchlen, s_term, c_searchlen, c_term);
            LM_DBG("Comparing [%.*s] and [%.*s]\n", alias_searchlen, s_term, c_searchlen, c_term);
        ...
        if (impucontact->contact)
            LM_DBG("Skipping %.*s\n", impucontact->contact->c.len, impucontact->contact->c.s);
