...
void revise_address(str *ip)
{
  ...
  LM_INFO("addr[0]: %c, addr[len-1]: %c (len: %d)\n", addr[0], addr[len-1], len);
  if (addr[0] == '[' && addr[len-1] == ']')
  {
    raddr = (char *)calloc(len -1, sizeof(char));
    strncpy(raddr, (addr + 1), len - 2);
    ip->s = raddr;
    ip->len = len - 2;
  }
  else
  ...
  LM_INFO("revise_address(): ip.s: %s, raddr: %s, ip.len: %d, len: %d\n", ip->s, raddr, ip->len, len);
}
//////////////////////////
...
uint16_t check_ip_version(str ip)
{
    ...
    LM_INFO("IP address: %s\n", ip.s);

    ///// Added for VoWiFi /////
    revise_address(&ip);
    LM_INFO("Revised IP address: %s\n", ip.s);
    ////////////////////////////
