...
import lte.statelearner.lteuesimulator.LTEUESimConfig;
import lte.statelearner.lteuesimulator.LTEUESimSUL;
import net.automatalib.automata.transout.MealyMachine;
import net.automatalib.util.graphs.dot.GraphDOT;
import net.automatalib.words.Word;
import net.automatalib.words.impl.SimpleAlphabet;
import lte.statelearner.LogOracle.MealyLogOracle;
import lte.statelearner.ModifiedWMethodEQOracle.MealyModifiedWMethodEQOracle;
import lte.statelearner.lteue.LTEUEConfig;
import lte.statelearner.lteue.LTEUESUL;
import lte.statelearner.smartcard.SCConfig;
import lte.statelearner.smartcard.SCSUL;
import lte.statelearner.socket.SocketConfig;
import lte.statelearner.socket.SocketSUL;
import lte.statelearner.tls.TLSConfig;
import lte.statelearner.tls.TLSSUL;
...
public class Learner {
	...
	public Learner(LearningConfig config) throws Exception {
		...
		else if(config.type == LearningConfig.TYPE_LTEUE) {
		...
		else if(config.type == LearningConfig.TYPE_LTEUESIM) {
			log.log(Level.INFO, "Using LTE-UE-SIM SUL");

			// Create the TLS SUL
			sul = new LTEUESimSUL(new LTEUESimConfig(config));
			alphabet = ((LTEUESimSUL)sul).getAlphabet();
		}
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			if (config.log_executor_active) {
				...
				LTEUEConfig lteueConfig = new LTEUEConfig(args[0]);
				LTEUESUL sul = new LTEUESUL(lteueConfig);
