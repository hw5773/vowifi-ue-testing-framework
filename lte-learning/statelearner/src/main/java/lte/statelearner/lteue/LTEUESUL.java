...
public class LTEUESUL implements StateLearnerSUL<String, String> {
	...
	//alphabet = enable_s1 identity_request_plain auth_request_plain security_mode_command  attach_accept identity_request_mac identity_request_wrong_mac tau_accept GUTI_reallocation dl_nas_transport rrc_release_tau identity_request_replay auth_request_replay security_mode_command_replay
	private static final List<String> enables1_expectedResults = Arrays.asList("attach_request", "null_action","attach_request_guti");
	private static final List<String> identityrequestplain_expectedResults = Arrays.asList("identity_response", "null_action");
	private static final List<String> identityrequestprotected_expectedResults = Arrays.asList("identity_response", "null_action");
	...
	private static final List<String> authrequestprotected_expectedResults = Arrays.asList("auth_response", "auth_failure_mac", "auth_failure_seq", "auth_failure_noneps", "null_action");
			...
			if (!config.combine_query) {
				...
				do {
					try {
						...
						result = mme_in.readLine();
						if (result == null || result.compareTo("")==0 || result_for_mme == null){
							continue;
						}
						//if(result_for_mme.contains("detach_request") || result_for_mme.contains("null_action") || result_for_mme.contains("attach_request")){
						//if(result_for_mme.contains("detach_request") || result_for_mme.contains("attach_request")){
						//	if (result_for_mme.contains("detach_request")){
						//		System.out.println("Caught detach");
						//		//result = mme_in.readLine();
						//		continue;
						//	}
						if(result_for_mme.contains("attach_request")){
							//send_enable_s1();
							//sleep(1000);
							result = mme_in.readLine();
							if (result == null || result.compareTo("")==0 || result_for_mme == null){
						...
						int attach_request_guti_counter = 5;
	...
	public boolean post_query_check() throws InterruptedException {
		...
		try {
			...
			if (result.contains("detach_request")) {
				attach_result = mme_in.readLine();
				if (attach_result.compareTo("") != 0 && attach_result.toCharArray()[0] == ' ') {
					attach_result = new String(Arrays.copyOfRange(result.getBytes(), 1, result.getBytes().length));
				}
				if(attach_result.contains("DONE")){
					attach_result = mme_in.readLine();
				}
				attach_result = getClosests(attach_result);
				//result = result + "," + attach_result;
			...
			System.out.println("Response of ENABLE_S1 in Post_query_check: " + result);
			mme_socket.setSoTimeout(5 * 1000);
			if(result.contains("attach_request")){
				return true;
			}
			else {
				return false;
	...
	public String reset_mme(){
		...
		try {
			...
			if (result1.compareTo("DONE") != 0) {
				...
				if (result1.compareTo("attachrequest") != 0) {
					//send_enable_s1();
					//sleep(2000);
				}
				//sleep(1000);
...
//            } while (!result.contains("DONE"));
	...
	public String getExpectedResult(String symbol, String result) {
		...
		else if (symbol.contains("dl_nas_transport") || symbol.contains("dl_nas_transport_plain") || symbol.contains("dl_nas_transport_replay")) {
		...
		else if (symbol.contains("GUTI_reallocation")||symbol.contains("GUTI_reallocation_plain") || symbol.contains("GUTI_reallocation_replay")) {
