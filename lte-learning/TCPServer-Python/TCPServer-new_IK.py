...
def airplane_mode_off():
    ...
    time.sleep(1)
    result1 = send_at_cmd("AT+CFUN=1", device, None, environment)
    #time.sleep(0.5)
...
def handle_reset(client_socket):
    ...
    #airplane_mode_on()
    #result = send_at_cmd("AT+CFUN=4,0", device, None, environment)
...
def Main():
    ...
    host = ""
    if (len(sys.argv)<3):
        print 'Usage: TCPServer-new.py <hostname> l,w <device name> s8plus, nexus6'
        exit()
