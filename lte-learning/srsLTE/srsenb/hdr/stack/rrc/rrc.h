...
public:
  ...
  void release_complete(uint16_t rnti);
    void release_complete_tau(uint16_t rnti);
  ...
  public:
    ...
    void send_connection_release();
      void send_connection_release_tau();
...
private:
  ...
  void process_release_complete(uint16_t rnti);
    ...
    void process_release_complete_tau(uint16_t rnti);
  ...
  const static uint32_t LCID_REL_USER2 = 0xffff0005;
