...
class s1ap
    ...
    ,public s1ap_interface_enodeb_statelearner
    ,public thread
{
public:
  s1ap();
  ~s1ap();
  bool init(s1ap_args_t args_, rrc_interface_s1ap *rrc_, enodeb_statelearner_interface_s1ap *enodeb_statelearner_, srslte::log *s1ap_log_); // Fuzzing
  void stop();
...
private:
  ...
  bool handle_uectxtreleasecommand(LIBLTE_S1AP_MESSAGE_UECONTEXTRELEASECOMMAND_STRUCT *msg);
    bool handle_uectxtreleasecommand_tau(LIBLTE_S1AP_MESSAGE_UECONTEXTRELEASECOMMAND_STRUCT *msg);
