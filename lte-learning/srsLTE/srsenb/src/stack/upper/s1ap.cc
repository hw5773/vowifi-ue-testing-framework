...
bool s1ap::init(s1ap_args_t args_, rrc_interface_s1ap *rrc_, enodeb_statelearner_interface_s1ap *enodeb_statelearner_, srslte::log *s1ap_log_) //Fuzzing
{
  rrc = rrc_;
  args = args_;
...
bool s1ap::user_release(uint16_t rnti, LIBLTE_S1AP_CAUSERADIONETWORK_ENUM cause_radio)
{
  ...
  cause.choice_type    = LIBLTE_S1AP_CAUSE_CHOICE_NAS;
  cause.choice.nas.ext = false;
  cause.choice.nas.e   = LIBLTE_S1AP_CAUSENAS_NORMAL_RELEASE;
...
bool s1ap:: execute_command(uint8_t* msg, uint16_t len){
  ...
  //s1ap_log->console("Received command\n");
  if(memcmp(msg, "rrc_release\n", len) == 0) {
    //s1ap_log->console("Release RRC Connections\n");
    //rrc->release_all_users();
      //handle_statelearner_query(FUZZING_MSG_TYPE_RRC_RELEASE);
  }
  uint8_t response[6] = "DONE\n";
  uint8_t size = 6;
  enodeb_statelearner->notify_response(response, size);
...
bool s1ap::handle_initiatingmessage(LIBLTE_S1AP_INITIATINGMESSAGE_STRUCT *msg)
{
  ...
  case LIBLTE_S1AP_INITIATINGMESSAGE_CHOICE_UECONTEXTRELEASECOMMAND:
    ...
    case LIBLTE_S1AP_INITIATINGMESSAGE_CHOICE_UECONTEXTRELEASECOMMAND_TAU:
        return handle_uectxtreleasecommand_tau(&msg->choice.UEContextReleaseCommand);
  ...
  default:
    s1ap_log->error("Unhandled intiating message: %s\n", liblte_s1ap_initiatingmessage_choice_text[msg->choice_type]);
          printf("Unhandled intiating message: %s\n", liblte_s1ap_initiatingmessage_choice_text[msg->choice_type]);
  }
  return true;
}
...
{
  s1ap_log->console("###### Received paging request from MME ##### \n");
  if(msg->ext) {
    s1ap_log->warning("Not handling S1AP message extension\n");
  }
...
bool s1ap::handle_uectxtreleasecommand(LIBLTE_S1AP_MESSAGE_UECONTEXTRELEASECOMMAND_STRUCT *msg)
{
  ...
  s1ap_log->console("Received UEContextReleaseCommand\n");
  ...
  if(msg->UE_S1AP_IDs.choice_type == LIBLTE_S1AP_UE_S1AP_IDS_CHOICE_UE_S1AP_ID_PAIR) {
    ...
    s1ap_log->info("Received UEContextReleaseCommand for RNTI = 0x%x\n", rnti);

    //ue_ctxt_map.erase(rnti); // Fuzzing
    //rrc->release_complete(rnti); // Fuzzing
  ...
  return true;
    ...
    bool s1ap::handle_uectxtreleasecommand_tau(LIBLTE_S1AP_MESSAGE_UECONTEXTRELEASECOMMAND_STRUCT *msg)
    {
        s1ap_log->info("Received UEContextReleaseCommand tau\n");

        s1ap_log->console("\n\nReceived UEContextReleaseCommand_tau\n");
...
bool s1ap::send_uectxtreleaserequest(uint16_t rnti, LIBLTE_S1AP_CAUSE_STRUCT *cause)
{
  ...
  //init->choice_type = LIBLTE_S1AP_CAUSERADIONETWORK_LOAD_BALANCING_TAU_REQUIRED;
...
bool s1ap::send_uectxtreleasecomplete(uint16_t rnti, uint32_t mme_ue_id, uint32_t enb_ue_id)
{
  ...
  tx_pdu.choice_type  = LIBLTE_S1AP_S1AP_PDU_CHOICE_SUCCESSFULOUTCOME;
   // tx_pdu.choice.initiatingMessage.choice.UEContextReleaseCommand.Cause.choice_type = LIBLTE_S1AP_CAUSE_CHOICE_NAS;
   // tx_pdu.choice.initiatingMessage.choice.UEContextReleaseCommand.Cause.choice.nas.ext = false;
    //tx_pdu.choice.initiatingMessage.choice.UEContextReleaseCommand.Cause.choice.nas.e = LIBLTE_S1AP_CAUSENAS_NORMAL_RELEASE;
  ...
  liblte_s1ap_pack_s1ap_pdu(&tx_pdu, (LIBLTE_BYTE_MSG_STRUCT*)&msg);
  s1ap_log->info_hex(msg.msg, msg.N_bytes, "Sending UEContextReleaseComplete for RNTI:0x%x\n", rnti);
    printf( "Sending UEContextReleaseComplete for RNTI:0x%x\n", rnti);
...
bool s1ap::send_initial_ctxt_setup_failure(uint16_t rnti)
{
  ...
  printf("KKKKKKKKKKKKKKKKIIIIIIIIIIIIII");
  //fail->Cause.choice.radioNetwork.e   = LIBLTE_S1AP_CAUSERADIONETWORK_UNSPECIFIED;
    fail->Cause.choice_type    = LIBLTE_S1AP_CAUSE_CHOICE_NAS;
    fail->Cause.choice.nas.ext = false;
    fail->Cause.choice.nas.e   = LIBLTE_S1AP_CAUSENAS_NORMAL_RELEASE;
  liblte_s1ap_pack_s1ap_pdu(&tx_pdu, (LIBLTE_BYTE_MSG_STRUCT*)buf.get());
