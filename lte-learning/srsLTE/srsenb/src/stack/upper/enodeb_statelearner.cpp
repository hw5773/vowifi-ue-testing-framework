...
namespace srsenb {
        ...
        while (running) {
            ...
            if(rd_sz == 0){
                ...
                if(memcmp(pdu->msg, "Hello\n", pdu->N_bytes) == 0) {
                    ...
                    //
                    //                    if (send(m_enodeb_statelearner_sock, "ACK\n", 4, 0) < 0)
                    //                    {
                    ...
                    //notify_response((uint8_t *) response.c_str(), response.size());
                }
                else {
    ...
    /*
    void
    enodeb_statelearner::cleanup(void) {
