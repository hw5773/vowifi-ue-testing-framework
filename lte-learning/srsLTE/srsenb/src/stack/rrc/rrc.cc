...
void rrc::release_complete(uint16_t rnti) {
  ...
  rx_pdu_queue.push(std::move(p));
    ...
    void rrc::release_complete_tau(uint16_t rnti) {

        rrc_pdu p = {rnti, LCID_REL_USER2, NULL};
        rx_pdu_queue.push(std::move(p));
    }
...
void rrc::add_paging_id(uint32_t ueid, LIBLTE_S1AP_UEPAGINGID_STRUCT UEPagingID) 
{
  ...
  if (pending_paging.count(ueid) == 0) {
    ...
    rrc_log->console("Received Paging for UEID=%d but not yet transmitted\n", ueid);
  }
  ...
  for (std::map<uint32_t, LIBLTE_S1AP_UEPAGINGID_STRUCT>::iterator iter = pending_paging.begin();
    ...
    if ((sfn % T) == (T / N) * (ueid % N)) {
      ...
      if (sf_idx < 0) {
        ...
        rrc_log->console("### Assembled paging for ue_id=%d, tti=%d ###\n", ueid, tti);
      }
    }
  }
...
void rrc::process_release_complete_tau(uint16_t rnti)
{
    printf("Iam here!!!");
  ...
  if (users.count(rnti) == 1) {
    if (!users[rnti].is_idle()) {
      ...
      users[rnti].send_connection_release_tau();
    ...
    if (users.count(p.rnti) == 1) {
      switch(p.lcid)
      {
        ...
        case LCID_REL_USER:
          ...
          break;
          case LCID_REL_USER2:
              process_release_complete_tau(p.rnti);
              break;
        case LCID_RLF_USER:
  ...
  while(running)
  {
    ...
    if (rem_rnti) {
      if (parent->s1ap->user_exists(rem_rnti)) {
         printf("IKKKKKKKKKKKKKKKKKKKKKKKK");
...
void rrc::ue::send_connection_release()
{
 ...
 dl_dcch_msg.msg.c1().rrc_conn_release().crit_exts.c1().rrc_conn_release_r8().release_cause = release_cause_e::other;
  ...
  send_dl_dcch(&dl_dcch_msg);
    ...
    void rrc::ue::send_connection_release_tau()
    {
        printf("IKK");
        dl_dcch_msg_s dl_dcch_msg;
        dl_dcch_msg.msg.set_c1().set_rrc_conn_release();
        ...
        dl_dcch_msg.msg.c1().rrc_conn_release().crit_exts.set_c1().set_rrc_conn_release_r8();
        //IK modified
       dl_dcch_msg.msg.c1().rrc_conn_release().crit_exts.c1().rrc_conn_release_r8().release_cause = release_cause_e::load_balancing_ta_urequired;
