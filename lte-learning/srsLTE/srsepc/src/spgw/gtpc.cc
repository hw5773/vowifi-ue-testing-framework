...
void spgw::gtpc::handle_s11_pdu(srslte::byte_buffer_t* msg)
{
  ...
  switch (pdu->header.type) {
    case srslte::GTPC_MSG_TYPE_CREATE_SESSION_REQUEST:
      ...
      break;
      case srslte::GTPC_MSG_TYPE_CREATE_SESSION_REQUEST_TAU:
          handle_create_session_request_tau(pdu->choice.create_session_request);
          break;
  ...
  return;
    ...
    void spgw::gtpc::handle_create_session_request_tau(const struct srslte::gtpc_create_session_request& cs_req)
    {
        ...
        header->type         = srslte::GTPC_MSG_TYPE_CREATE_SESSION_RESPONSE_TAU;
...
spgw_tunnel_ctx_t* spgw::gtpc::create_gtpc_ctx(const struct srslte::gtpc_create_session_request& cs_req)
{
  ...
  in_addr_t ue_ip;
  //----------------------Fuzzing: allocating a fixed ip address --------------------
  unsigned char * ip;

  //have ip point to s_addr
  ip = (unsigned char *) &(ue_ip);

  //set the bytes for "172.16.0.225"
  ip[0]=172;
  ip[1]=16;
  ip[2]=0;
  ip[3]=225;
