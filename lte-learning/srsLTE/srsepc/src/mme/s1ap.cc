...
int s1ap::init(s1ap_args_t s1ap_args, srslte::log_filter* nas_log, srslte::log_filter* s1ap_log, mme_statelearner_interface_s1ap *mme_statelearner_) //Fuzzing
{
  ...
  m_s1ap_nas_transport->init(mme_statelearner_);
...
//===================================== Fuzzing ======================================
    bool s1ap::send_query(uint8_t *msg, uint16_t len) {
        ...
        /*
        if(rcvd_msg.find("Query")){
            printf("%s", rcvd_msg.c_str());
        ...
        */
        ...
        if(memcmp(msg, "RESET", 5) == 0){
            ...
            m_s1ap_log->console("MME_STATE = RESET START\n");
            m_s1ap_log->console("# query: RESET #\n");
            m_s1ap_log->console("%s", rcvd_msg.c_str());

            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_RESET_UL_NAS_COUNT);
            ...
            if(nas_ctx != NULL) {
                ...
                if (ret == false) {
                    m_s1ap_log->console("ERROR in Resetting ue_ctx\n");
                }
            }

            m_tmsi_to_imsi.clear();
            //      uint8_t response[] = "DONE\n";
            //      uint8_t size = *(&response + 1) - response;
            uint8_t response[6] = "DONE\n";
            uint8_t size = 6;
            m_mme_statelearner->notify_response(response, size);
            ...
            ret = true;
            //m_pool->deallocate(reply_buffer);
            m_s1ap_log->console("MME_STATE = RESET END\n");
            m_mme_statelearner->reset_mme_statelearner_reset_state();
            pthread_mutex_unlock(&s1ap_instance_mutex);
        }

        else if(memcmp(msg, "enable_s1\n", len) == 0){
            pthread_mutex_lock(&s1ap_instance_mutex);

            m_s1ap_log->console("###query: enable_s1 ###\n");
            // NOTE: NOT SENDING ANY RESPONSE FOR ENABLE_S1
            uint8_t response[6] = "DONE\n";
            uint8_t size = 6;
            m_mme_statelearner->notify_response(response, size);
            ret = true;
        ...
        else if(memcmp(msg, "auth_request_plain\n", len) == 0){
            m_s1ap_log->console("###query: auth_request_plain ###\n");
            //ret = m_s1ap_nas_transport->handle_statelearner_query_authentication_request();
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_AUTH_REQUEST);
        }
        /////////////////////////////////////////IK///////////////////////////////////
        else if(memcmp(msg, "auth_request_mac\n", len) == 0){
            m_s1ap_log->console("###query: auth_request_mac ###\n");
            //ret = m_s1ap_nas_transport->handle_statelearner_query_authentication_request();
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_AUTH_REQUEST_MAC);
        }
        else if(memcmp(msg, "auth_request_protected\n", len) == 0){
            m_s1ap_log->console("###query: auth_request_protected ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_AUTH_REQUEST_ENCRYPT_MAC);
        }
        else if(memcmp(msg, "auth_request_wrong_mac\n", len) == 0){
            m_s1ap_log->console("###query: auth_request_wrong_mac ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_AUTH_REQUEST_WMAC);
        }
        else if(memcmp(msg, "auth_request_encrypt\n", len) == 0){
            m_s1ap_log->console("###query: auth_request_encrypt ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_AUTH_REQUEST_ENCRYPT);
        }
        else if(memcmp(msg, "auth_request_replay\n", len) == 0){
            m_s1ap_log->console("###query: auth_request_replay ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_AUTH_REQUEST_REPLAY);
        }
        ///////////////////////////////////////////IK//////////////////////////////
        else if(memcmp(msg, "auth_reject\n", len) == 0){
            m_s1ap_log->console("###query: auth_reject ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_AUTH_REJECT);
        }
        else if(memcmp(msg, "attach_reject\n", len) == 0){
            m_s1ap_log->console("###query: attach_reject ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_ATTACH_REJECT);
        }
        else if(memcmp(msg, "identity_request_plain\n", len) == 0){
            m_s1ap_log->console("###query: identity_request_plain ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_IDENTITY_REQUEST);
        }
        ////////////////////////////////////////////////////////IK/////////////////////////////////////////////////////
        else if(memcmp(msg, "identity_request_mac\n", len) == 0){
            m_s1ap_log->console("###query: identity_request_mac ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_IDENTITY_REQUEST_MAC);
        }
        else if(memcmp(msg, "identity_request_protected\n", len) == 0){
            m_s1ap_log->console("###query: identity_request_protected ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_IDENTITY_REQUEST_ENCRYPT_MAC);
        }
        else if(memcmp(msg, "identity_request_wrong_mac\n", len) == 0){
            m_s1ap_log->console("###query: identity_request_wrong_mac ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_IDENTITY_REQUEST_WMAC);
        }
        else if(memcmp(msg, "identity_request_encrypt\n", len) == 0){
            m_s1ap_log->console("###query: identity_request_encrypt ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_IDENTITY_REQUEST_ENCRYPT);
        }
        else if(memcmp(msg, "identity_request_replay\n", len) == 0){
            m_s1ap_log->console("###query: identity_request_replay ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_IDENTITY_REQUEST_REPLAY);
        }
        ///////////////////////////////////////////////////////IK////////////////////////////////////////////
        else if(memcmp(msg, "security_mode_command\n", len) == 0){
            m_s1ap_log->console("### query: security_mode_command ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_SEC_MOD_CMD);
        }
        else if(memcmp(msg, "esm_info_request\n", len) == 0){
            m_s1ap_log->console("### query: esm_info_request ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_ESM_INFO_REQUEST);
        }
        else if(memcmp(msg, "attach_accept\n", len) == 0){
            m_s1ap_log->console("### query: attach_accept ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_ATTACH_ACCEPT);
        }
        else if(memcmp(msg, "tau_accept\n", len) == 0){
            m_s1ap_log->console("### query: tau_accept ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_TAU_ACCEPT);
        }
        else if(memcmp(msg, "tau_accept_plain\n", len) == 0){
          m_s1ap_log->console("### query: tau_accept_plain ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_TAU_ACCEPT_PLAIN);
        }
        ////////////////////////////////IK/////////////////////////////////////////////
        else if(memcmp(msg, "attach_accept_mac\n", len) == 0){
            m_s1ap_log->console("###query: attach_accept_mac ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_ATTACH_ACCEPT_MAC);
        }
        else if(memcmp(msg, "attach_accept_wmac\n", len) == 0){
            m_s1ap_log->console("###query: attach_accept_wmac ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_ATTACH_ACCEPT_WMAC);
        }
        else if(memcmp(msg, "attach_accept_replay\n", len) == 0){
            m_s1ap_log->console("###query: attach_accept_replayed_mac ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_ATTACH_ACCEPT_REPLAY);
        }
        else if(memcmp(msg, "dl_nas_transport\n", len) == 0){
            m_s1ap_log->console("###query: dl_nas_transport ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_DL_NAS_TRANSPORT);
        }
        else if(memcmp(msg, "dl_nas_transport_plain\n", len) == 0){
          m_s1ap_log->console("###query: dl_nas_transport_plain ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_DL_NAS_TRANSPORT_PLAIN);
        }
        else if(memcmp(msg, "dl_nas_transport_replay\n", len) == 0){
          m_s1ap_log->console("###query: dl_nas_transport_replay ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_DL_NAS_TRANSPORT_REPLAY);
        }
        //////////////////////////////////IK///////////////////////////////////////////
        else if(memcmp(msg, "emm_info\n", len) == 0){
            m_s1ap_log->console("### query: emm_info ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_EMM_INFO);
        }
        else if(memcmp(msg, "detach_request\n", len) == 0){
            m_s1ap_log->console("### query: detach_request ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_DETACH_REQUEST);
        }
        else if(memcmp(msg, "GUTI_reallocation\n", len) == 0){
            m_s1ap_log->console("### query: GUTI_rellocation ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_GUTI_REALLOCATION);
        }
        else if(memcmp(msg, "GUTI_reallocation_plain\n", len) == 0){
          m_s1ap_log->console("### query: GUTI_rellocation_plain ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_GUTI_REALLOCATION_PLAIN);
        }
        else if(memcmp(msg, "GUTI_reallocation_replay\n", len) == 0){
          m_s1ap_log->console("### query: GUTI_rellocation_replay ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_GUTI_REALLOCATION_REPLAY);
        }
        else if(memcmp(msg,"security_mode_command_replay\n",len)==0){
          m_s1ap_log->console("### query: security_mode_command_replay ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_SECURITY_MODE_COMMAND_REPLAY);
        }
        else if(memcmp(msg, "rrc_release\n", len) == 0){
          
          m_s1ap_log->console("### query: rrc_release ###\n");
            nas* nas_ctx = find_nas_ctx_from_imsi(m_s1ap_args.ue_under_test_imsi);
            if (nas_ctx == NULL) {
              m_s1ap_log->error("Error finding NAS context when sending UE Context Setup Release\n");
              return false;
            }
            m_s1ap_ctx_mngmt_proc->handle_statelearner_ue_context_release_request(nas_ctx, &nas_ctx->m_ecm_ctx.enb_sri);
            ret = true;
        }
        else if(memcmp(msg, "rrc_release_tau\n", len) == 0){

            m_s1ap_log->console("### query: rrc_release_tau ###\n");
            nas* nas_ctx = find_nas_ctx_from_imsi(m_s1ap_args.ue_under_test_imsi);
            if (nas_ctx == NULL) {
                m_s1ap_log->error("Error finding NAS context when sending UE Context Setup Release\n");
                return false;
            }
            m_s1ap_ctx_mngmt_proc->handle_statelearner_ue_context_release_request_tau(nas_ctx, &nas_ctx->m_ecm_ctx.enb_sri);
            ret = true;
        }
        else if(memcmp(msg, "paging_tmsi\n", len) == 0){

          m_s1ap_log->console("### query: paging_tmsi ###\n");
          m_s1ap_paging->send_paging(m_s1ap_args.ue_under_test_imsi, 0);
          ret = true;
