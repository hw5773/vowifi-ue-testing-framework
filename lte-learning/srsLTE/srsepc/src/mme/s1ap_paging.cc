...
bool s1ap_paging::send_paging(uint64_t imsi, uint16_t erab_to_setup) {
  ...
  if (nas_ctx == NULL) {
    ...
    m_s1ap_log->console("Could not find UE to page NAS context\n");
    return false;
  }
  ...
  if (m_s1ap->m_s1ap_args.ue_state_fuzzing_enable == false){
    // Start T3413
  ...
  m_s1ap_log->console("###### Paging UE -- M-TMSI :0x%x ####\n", m_tmsi);
  ...
  m_s1ap_log->console("# Paging sent! #\n");
  return true;
}
