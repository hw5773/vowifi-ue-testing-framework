...
namespace srsepc {
    ...
    bool s1ap_ctx_mngmt_proc::send_initial_context_setup_request(nas* nas_ctx, uint16_t erab_to_setup)
    {
        ...
        if (emm_ctx->state == EMM_STATE_DEREGISTERED) { // Fuzzing: Commenting it to enable to send attach_accept even the UE is registered.
    ...
    bool s1ap_ctx_mngmt_proc::send_initial_context_setup_request_tau(nas* nas_ctx, uint16_t erab_to_setup)
    {
        // Prepare reply PDU
        LIBLTE_S1AP_S1AP_PDU_STRUCT pdu;
        bzero(&pdu, sizeof(LIBLTE_S1AP_S1AP_PDU_STRUCT));
        ...
        m_s1ap_log->console("Adding attach accept to Initial Context Setup Request\n");
        m_s1ap_log->info("Adding attach accept to Initial Context Setup Request\n");
        nas_ctx->pack_tau_accept(nas_buffer);
...
//==================================================== Fuzzing =======================================
    bool s1ap_ctx_mngmt_proc::handle_statelearner_ue_context_release_request(srsepc::nas *nas_ctx,
                                                                             struct sctp_sndrcvinfo *enb_sri){
        LIBLTE_S1AP_MESSAGE_UECONTEXTRELEASEREQUEST_STRUCT ue_rel_req;
        ...
        //nas* nas_ctx = m_s1ap->find_nas_ctx_from_mme_ue_s1ap_id(mme_ue_s1ap_id);
        if (nas_ctx == NULL) {
            m_s1ap_log->info("No UE context to release found");
            m_s1ap_log->console("No UE context to release found");
            return false;
        }
        ...
        uint32_t mme_ue_s1ap_id = ecm_ctx->mme_ue_s1ap_id;
    ...
    bool s1ap_ctx_mngmt_proc::handle_statelearner_ue_context_release_request_tau(srsepc::nas *nas_ctx,
                                                                             struct sctp_sndrcvinfo *enb_sri){
        LIBLTE_S1AP_MESSAGE_UECONTEXTRELEASEREQUEST_STRUCT ue_rel_req;
        ...
        if (ecm_ctx->state == ECM_STATE_CONNECTED) {
            ...
            // Send release context command to enb, so that it can release it's bearers
            send_ue_context_release_command_tau(nas_ctx);
        } else {
            // No ECM Context to release
            m_s1ap_log->info("UE is not ECM connected. No need to release S1-U. MME UE S1AP Id %d\n", mme_ue_s1ap_id);
            m_s1ap_log->console("UE is not ECM connected. No need to release S1-U. MME UE S1AP Id %d\n", mme_ue_s1ap_id);
...
//=====================================================================================================
    ...
    bool s1ap_ctx_mngmt_proc::send_ue_context_release_command(nas* nas_ctx)
    {
        ...
        //ctx_rel_cmd->Cause.choice.nas.ext = false;
        //ctx_rel_cmd->Cause.choice.nas.e   = LIBLTE_S1AP_CAUSENAS_NORMAL_RELEASE;
        //ctx_rel_cmd->Cause.choice.radioNetwork.e  = LIBLTE_S1AP_CAUSERADIONETWORK_LOAD_BALANCING_TAU_REQUIRED;
    ...
    bool s1ap_ctx_mngmt_proc::send_ue_context_release_command_tau(nas* nas_ctx)
    {
        srslte::byte_buffer_t* reply_buffer = m_pool->allocate();
        ...
        LIBLTE_S1AP_INITIATINGMESSAGE_STRUCT* init = &pdu.choice.initiatingMessage;
        init->procedureCode                        = LIBLTE_S1AP_PROC_ID_UECONTEXTRELEASE_TAU;
        init->choice_type                          = LIBLTE_S1AP_INITIATINGMESSAGE_CHOICE_UECONTEXTRELEASECOMMAND_TAU;
        ...
        LIBLTE_ERROR_ENUM err = liblte_s1ap_pack_s1ap_pdu_tau(&pdu, (LIBLTE_BYTE_MSG_STRUCT*)reply_buffer);
        if (err != LIBLTE_SUCCESS) {
            m_s1ap_log->error("Could not pack Context Release Command Message\n");
            m_pool->deallocate(reply_buffer);
            return false;
