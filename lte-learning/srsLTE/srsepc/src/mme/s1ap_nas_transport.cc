...
void s1ap_nas_transport::init(mme_statelearner_interface_s1ap *mme_statelearner_) //Fuzzing
{
  ...
  m_nas_init.ue_under_test_imsi = m_s1ap->m_s1ap_args.ue_under_test_imsi; // Fuzzing
...
//================================================== Fuzzing =========================================
  bool s1ap_nas_transport:: handle_statelearner_query(uint8_t  msg_type){
      ...
      else if(msg_type == FUZZING_MSG_TYPE_AUTH_REJECT){
        return nas_ctx->handle_statelearner_query_authentication_reject();
      }
      else if(msg_type == FUZZING_MSG_TYPE_ESM_INFO_REQUEST){
        return nas_ctx->handle_statelearner_query_esm_information_request();
      }
      else if(msg_type == FUZZING_MSG_TYPE_EMM_INFO){
        return nas_ctx->handle_statelearner_query_emm_information();
      }
      else if(msg_type == FUZZING_MSG_TYPE_PAGING_TMSI){
        return nas_ctx->handle_statelearner_query_paging_with_tmsi();
      }
      ////////////////////////IK//////////////////////////
                                                     ...
                                                     bool*                                          reply_flag)
{
  ...
  ecm_ctx->enb_ue_s1ap_id = enb_ue_s1ap_id; // Fuzzing
  ...
  if (sec_hdr_type == LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY ||
    ...
    if (mac_valid == false) {
      m_s1ap_log->warning("Invalid MAC message. Even if security header indicates integrity protection (Maybe: "
        ...
        //m_s1ap_log->console("$$$$$$$$$$$$$ MAC Verified Message $$$$$$$$$$$$$$$$\n");
        MAC = true;
    }
  }
  ...
  if (sec_hdr_type == LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY_AND_CIPHERED ||
    ...
    msg_encrypted = true;
    //m_s1ap_log->console("$$$$$$$$$$$$$ Message Decrypted $$$$$$$$$$$$$$$$\n");
  ...
  if (sec_hdr_type != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
    ...
    if (emm_ctx->imsi == 0) {
      ...
      flag = true;
        m_s1ap_log->console("$$$$$$$$$$$$$ Setting flag $$$$$$$$$$$$$$$$\n");
  ...
  switch (msg_type) {
    ...
    case LIBLTE_MME_MSG_TYPE_EMM_STATUS:
      ...
      //m_s1ap_log->console("\nPlain Protected UL NAS: EMM Status\n");
    ...
    case LIBLTE_MME_MSG_TYPE_TRACKING_AREA_UPDATE_COMPLETE:
      ...
      //} else {
        // Attach Complete was not integrity protected
        //m_s1ap_log->console("TAU Complete not integrity protected. Discard message.\n");
        //m_s1ap_log->warning("TAU Complete not integrity protected. Discard message.\n");
        // increase_ul_nas_cnt = false;
      //}
      break;
      ...
      case LIBLTE_MME_MSG_TYPE_UPLINK_NAS_TRANSPORT:
          ...
          if (ul_nas_transport%2!=0){
              ...
              m_s1ap_log->console("IK accepting ul_nas_transport\n");
              printf("ul_nas_count IKKKKKKKKKKKKKKKKKK: %d\n",ul_nas_transport);
          }else{
              m_s1ap_log->console("IK Discarding ul_nas_transport\n");
          }
          break;
