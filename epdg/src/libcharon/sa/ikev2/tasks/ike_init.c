...
static void report_sa_payload(sa_payload_t *sa_payload, instance_t *instance, uint64_t ispi, uint64_t rspi);
////////////////////////////
...
static bool build_payloads(private_ike_init_t *this, message_t *message)
{
  ...
  ////////////////////////////
    ...
    if (check_instance(instance, ispi, rspi, NON_UPDATE))
    {
      ...
      printf("[VoWiFi] ike_init.c: before modification: algo: %u, klen: %u\n", *algo, *klen);
...
static void process_payloads(private_ike_init_t *this, message_t *message)
{
  ...
  ///////////////////////////
	...
	while (enumerator->enumerate(enumerator, &payload))
	{
        ...
        if (check_instance(instance, ispi, rspi, NON_UPDATE))
          report_sa_payload((sa_payload_t *)payload, instance, ispi, rspi);
				break;
			}
